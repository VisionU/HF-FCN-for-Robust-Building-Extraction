...
import argparse
import caffe
from test_prediction import get_predict
...
def create_patches(sat_patch_size, map_patch_size, stride, map_ch,
    ...
    # get filenames
    sat_fns = np.asarray(sorted(glob.glob('%s/*.npy' % sat_data_dir)))
...
if __name__ == '__main__':
    ...
    n_iter = int(args.weight.split('_')[-1].split('.')[0])
    out_dir = os.path.dirname(args.model)
    out_dir = '%s/train_data_%d' % (out_dir, n_iter)
    ...
    train_sat_dir = 'data/mass_merged/train/sat'
    for fn in glob.glob('%s/*.tiff' % train_sat_dir):
        ortho = cv.imread(fn)
        pred_img, ortho_img = get_predict(ortho, net, num,
                                          l_ch, l_height, l_width,
                                          d_ch, d_height, d_width)
        out_fn = os.path.basename(fn).split('.')[0]
        np.save('%s/%s.npy' % (out_dir, out_fn), pred_img)
        print out_fn
